...
(function() {
  ...
  exports.compile = compile = withPrettyErrors(function(code, options) {
    var currentColumn, currentLine, encoded, extend, filename, fragment, fragments, generateSourceMap, header, i, j, js, jsxFrameworks, k, len, len1, len2, map, merge, newLines, ref, ref1, ref2, sourceMapDataURI, sourceURL, token, tokens, v3SourceMap;
    ...
    if (options.jsxFramework == null) {
      options.jsxFramework = 'react';
    }
    options.jsxFrameworkOptions = jsxFrameworks[options.jsxFramework];
    for (j = 0, len1 = tokens.length; j < len1; j++) {
      token = tokens[j];
      if ((ref1 = token[0]) === 'JSX_START_TAG_START' || ref1 === 'JSX_ELEMENT_NAME') {
        options.containsJsx = true;
        if (options.jsxFrameworkOptions.jsxImport != null) {
          (options.jsxImports != null ? options.jsxImports : options.jsxImports = []).push(options.jsxFrameworkOptions.jsxImport);
        }
        break;
      }
    }
