...
(function() {
  ...
  exports.Lexer = Lexer = class Lexer {
    ...
    jsxToken() {
      var consumedChunk, originalChunk, originalChunkColumn, originalChunkLine;
      originalChunk = this.chunk;
      [originalChunkLine, originalChunkColumn] = [this.chunkLine, this.chunkColumn];
      if (!this.matchJsxElement({
        topLevel: true
      })) {
        return 0;
      }
      [this.chunkLine, this.chunkColumn] = [originalChunkLine, originalChunkColumn];
      consumedChunk = this.chunk;
      this.chunk = originalChunk;
      return this.chunk.length - consumedChunk.length;
    }
    ...
    matchJsxTagBody({elementName}) {
      var consumed, endTag, errorExpectedEndTag, followsNewline, followsWhitespace, indentedBody, justOutdented, noMatch, numOutdents, originalIndent, outdentNext, popLevels;
    ...
    matchJsxClassShorthand(opts = {}) {
      var addImplicitElementName, allowLeadingWhitespace, close, column, index, isInterpreted, isStyle, klass, line, match, nested, open, ref, ref1, regex, symbol;
      ({addImplicitElementName, allowLeadingWhitespace} = opts);
      ...
      match[0], symbol = match[1], isStyle = match[2], isInterpreted = match[3], klass = match[4];
    ...
    matchJsxIndentedBody({elementName, topLevel}) {
      var consumed, followsNewline, followsWhitespace, noMatch, numOutdents, popLevels;
      this.token('JSX_ELEMENT_BODY_START', elementName, 0, 0);
    ...
    jsxForceExpression({nested, inset = 0}) {
      ...
      if (!((function() {
        ...
        for (j = 0, len = nested.length; j < len; j++) {
          token = nested[j];
          if ((ref = token[0]) === 'FOR' || ref === 'SWITCH' || ref === 'WHILE' || ref === 'UNTIL' || ref === 'IF' || ref === 'POST_IF') {
            results.push(token);
          }
        }
        return results;
      ...
      nested.splice(inset, 0, this.makeToken('(', '(', 0, 0));
    ...
    lineToken(opts = {}) {
      ...
      switch (action) {
        ...
        default:
          this.indebt = 0;
          numOutdents = this.outdentToken(this.indent - size, noNewlines, indent.length, {includesBlankLine});
    ...
    outdentToken(moveOut, noNewlines, outdentLength, {includesBlankLine} = {}) {
      var decreasedIndent, dent, lastIndent, numOutdents, ref, token;
