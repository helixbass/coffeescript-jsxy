...
(function() {
  ...
  exports.Block = Block = (function() {
    class Block extends Base {
      ...
      generateJsxImports(o) {
        var addedDynamicClassNamesImport, alreadyExplicitlyImported, explicitImports, j, jsxImport, jsxImports, len1;
        ({jsxImports = []} = o);
        addedDynamicClassNamesImport = false;
        this.traverseChildren(true, (child) => {
          var ref1;
          if (addedDynamicClassNamesImport) {
            return;
          }
          if (!(child instanceof JsxElement)) {
            return;
          }
          if (child.shorthands.classes.some(function(klass) {
            return Array.isArray(klass);
          })) {
            addedDynamicClassNamesImport = true;
            if (((ref1 = o.jsxFrameworkOptions.dynamicClassNamesFunction) != null ? ref1.from : void 0) != null) {
              return jsxImports.push(o.jsxFrameworkOptions.dynamicClassNamesFunction);
            }
          }
        });
        explicitImports = [];
        this.traverseChildren(false, (child) => {
          ...
          return explicitImports.push(child);
        });
        alreadyExplicitlyImported = function({importDefault, from}) {
          var clause, j, len1, ref1, ref2, source;
          for (j = 0, len1 = explicitImports.length; j < len1; j++) {
            ({clause, source} = explicitImports[j]);
            if (importDefault !== (clause != null ? (ref1 = clause.defaultBinding) != null ? ref1.identifier : void 0 : void 0)) {
              continue;
            }
            if (from !== (source != null ? (ref2 = source.value) != null ? ref2.slice(1, -1) : void 0 : void 0)) {
              continue;
            }
            return true;
          }
        };
        for (j = 0, len1 = jsxImports.length; j < len1; j++) {
          jsxImport = jsxImports[j];
          if (!alreadyExplicitlyImported(jsxImport)) {
            this.expressions.unshift(new ImportDeclaration(new ImportClause(new IdentifierLiteral(jsxImport.importDefault)), new StringLiteral(`'${jsxImport.from}'`)));
          }
        }
        if (jsxImports.length) {
          return o.bare = true;
        }
      }
  ...
  exports.Value = Value = (function() {
    class Value extends Base {
      ...
      compileNode(o) {
        ...
        this.base.jsxAttributeName = this.jsxAttributeName;
  ...
  exports.JsxElement = JsxElement = (function() {
    class JsxElement extends Base {
      constructor(options) {
        ...
        this.inlineTagBody = this.children_.inlineBody;
        if ((base1 = this.attributes).list == null) {
          base1.list = [];
        }
        ({
          list: this.listAttributes,
          object: this.objAttributes
        } = this.attributes);
        if ((base2 = this.shorthands).classes == null) {
          base2.classes = [];
        }
      }

      compileNode(o) {
        var compiledChildren, compiledClassAttribute, compiledIdAttribute, compiledListAttributes, compiledObjectAttributes, endTag, idt, initialIndent, isString, ref1, ref2, startTag;
        compiledIdAttribute = (() => {
          if (!this.shorthands.id) {
            return [];
          }
          return [this.makeCode(` id='${this.shorthands.id}'`)];
        })();
        compiledClassAttribute = (() => {
          var _extends, classNameAttribute, classes, combined, dynamicClassNamesFunction, hasInterpretedClasses, j, klass, len1, styleArray, styleShorthand;
          ({classes} = this.shorthands);
          if (!classes.length) {
            return [];
          }
          hasInterpretedClasses = classes.some(function(klass) {
            return Array.isArray(klass);
          });
          ({classNameAttribute = 'className', dynamicClassNamesFunction, styleArray} = o.jsxFrameworkOptions);
          styleShorthand = (function() {
            var j, klass, len1;
            for (j = 0, len1 = classes.length; j < len1; j++) {
              klass = classes[j];
              if (klass instanceof Arr) {
                return klass;
              }
            }
          })();
          if (styleShorthand != null) {
            ...
            (this.objAttributes != null ? this.objAttributes : this.objAttributes = new JsxAttributesObj).properties.push(new Assign(new Value(new IdentifierLiteral('style')), styleShorthand.objects.length > 1 ? styleArray === 'merge' ? (_extends = new Value(new Literal(utility('_extends', o))), new Call(_extends, [new Obj, ...styleShorthand.objects])) : styleShorthand : styleShorthand.objects[0], 'object', new Literal(':')));
          }
          if (!classes.length) {
            return [];
          }
          if (!hasInterpretedClasses) {
            return [this.makeCode(` ${classNameAttribute}='${classes.join(' ')}'`)];
          }
          combined = [];
          for (j = 0, len1 = classes.length; j < len1; j++) {
            klass = classes[j];
          ...
          (this.objAttributes != null ? this.objAttributes : this.objAttributes = new JsxAttributesObj).properties.push(new Assign(new Value(new IdentifierLiteral(classNameAttribute)), dynamicClassNamesFunction === 'array' ? new Arr(combined) : new Call(new Value(new IdentifierLiteral(dynamicClassNamesFunction.importDefault)), combined), 'object', new Literal(':')));
          return [];
        })();
        compiledListAttributes = (() => {
          var attr, j, len1, name, ref1, value;
          if (!this.listAttributes.length) {
            return [];
          }
          attr = [];
          ref1 = this.listAttributes;
          for (j = 0, len1 = ref1.length; j < len1; j++) {
            ({name, value} = ref1[j]);
            attr.push(this.makeCode(' '));
            attr.push(this.makeCode(name));
            attr.push(this.makeCode('='));
            if (value instanceof StringLiteral) {
              attr.push(...value.compileToFragments(o)); // expression value
            } else {
              attr.push(this.makeCode('{'));
        ...
        compiledObjectAttributes = (ref1 = (ref2 = this.objAttributes) != null ? typeof ref2.compileToFragments === "function" ? ref2.compileToFragments(o) : void 0 : void 0) != null ? ref1 : [];
        startTag = [this.makeCode('<'), this.makeCode(this.name), ...compiledIdAttribute, ...compiledClassAttribute, ...compiledListAttributes, ...compiledObjectAttributes, this.makeCode('>')];
        isString = function(obj) {
          return Object.prototype.toString.call(obj) === '[object String]';
        };
        initialIndent = o.indent;
        compiledChildren = (() => {
          var child, compiled, idt, index;
          ...
          compiled = flatten((function() {
            ...
            for (index = j = 0, len1 = ref3.length; j < len1; index = ++j) {
              child = ref3[index];
              results.push([
                this.makeCode(this.inlineTagBody ? '' : child.inline && index ? child.inline === 'IMMEDIATE' ? '' : ' ' : index && this.children_[index - 1].inline === 'both' ? ' ' : `\n${idt}`),
                ...(child instanceof JsxContent ? [this.makeCode(child.str)] : child instanceof JsxInlineContent ? [this.makeCode(child.str)] : child instanceof JsxElement ? child.compileToFragments(o) : [this.makeCode('{'), // expression
        ...
        if (this.children_.length && !this.inlineTagBody) {
          endTag.unshift(this.makeCode(`\n${idt}`));
        }
        return [...startTag, ...compiledChildren, ...endTag];
      }

    };

    JsxElement.prototype.children = ['children_', 'objAttributes'];

    return JsxElement;

  })();

  exports.JsxContent = JsxContent = class JsxContent extends Base {
    constructor(str) {
      super();
      this.str = str;
    }
  ...
  exports.JsxInlineContent = JsxInlineContent = class JsxInlineContent extends Base {
    constructor(str) {
      super();
      this.str = str;
      this.inline = 'IMMEDIATE';
    }
  ...
  exports.JsxAttributesObj = JsxAttributesObj = class JsxAttributesObj extends Obj {
    compileNode(o) {
      var answer, compiled, j, len1, prop, ref1;
      answer = [];
      ref1 = this.properties;
      for (j = 0, len1 = ref1.length; j < len1; j++) {
        prop = ref1[j];
        ...
        if (prop instanceof Splat) {
          compiled = [this.makeCode('{'), ...compiled, this.makeCode('}')];
        }
    ...
    class Assign extends Base {
      ...
      compileNode(o) {
        ...
        if (this.jsxAttribute) {
          val = [this.makeCode('{'), ...val, this.makeCode('}')];
        }
        if (this.jsxAttribute) {
          this.variable.jsxAttributeName = true;
        }
  ...
  exports.StringWithInterpolations = StringWithInterpolations = (function() {
    class StringWithInterpolations extends Base {
      ...
      compileNode(o) {
        var code, element, elements, expr, fragments, j, len1, salvagedComments;
